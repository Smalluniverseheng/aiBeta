...
// 预设角色横幅：清除角色
  ...
  (function initFloatBackBtn() {
    var floatBtn = document.getElementById('floatBackBtn');
    if (!floatBtn) return;
    ...
    function updateFloatBtnVisibility() {
      var device = document.documentElement.getAttribute('data-device') || 'desktop';
      var navMode = Store.state.navSettings && Store.state.navSettings[device] ? Store.state.navSettings[device] : 'navbar';
      var isChatPage = document.querySelector('.chat-scroll') && document.querySelector('.chat-scroll').offsetParent !== null;

      // 只在手表端且设置为 float 且不在对话页面时显示
      if (device === 'watch' && navMode === 'float' && !isChatPage) {
    ...
    floatBtn.addEventListener('click', function(e) {
      if (hasMoved) return; // 如果拖动了，不触发点击
      e.preventDefault();
      e.stopPropagation();
      navigate('chat');
    });
    ...
    floatBtn.addEventListener('touchmove', function(e) {
      ...
      floatBtn.style.left = (btnX + dx) + 'px';
      floatBtn.style.top = (btnY + dy) + 'px';
    ...
    floatBtn.addEventListener('touchend', function() {
      isDragging = false;
      floatBtn.style.transition = 'transform 0.15s, opacity 0.2s';
      // 吸附到边缘
      var rect = floatBtn.getBoundingClientRect();
      ...
      // 上下边界限制
      if (centerY < 60) {
        floatBtn.style.top = '60px';
        floatBtn.style.bottom = 'auto';
      } else if (centerY > vh - 60) {
    ...
    window.addEventListener('navigate', function() {
      setTimeout(updateFloatBtnVisibility, 100);
    });
